...
class QGPSLearning(SupervisedLearning):
    ...
    def set_kernel_mat(self, ref_site, confs):
        if self.K is None:
        ...
        if self.K.dtype == complex:
            ...
            complex_par = False
        self.K = self.kernel_mat_inner(self.machine._epsilon, ref_site, confs, self.K, complex_par, self.machine._Smap, self.machine._Smap_inverse, self.machine._sym_spin_flip_sign)
        return self.K

    def setup_fit_alpha_dep(self, confset, target_amplitudes, ref_site, alpha_mat):
    ...
    def setup_fit(self, confset, target_amplitudes, ref_site, noise_tilde, alpha_mat):
        self.log_amps = target_amplitudes.astype(self.machine._epsilon.dtype)
        self.fit_data = np.log(self.log_amps)
        self.set_kernel_mat(ref_site, confset)
    ...
    def log_marg_lik_alpha_der(self):
        derivative_alpha = 1/self.alpha_mat
        if self.cholesky:
    ...
    def fit_step(self, confset, target_amplitudes, ref_site, noise_tilde, alpha, noise_range=(1.e-10, 1.e5), alpha_range=(1.e-10, 1.e7),
        ...
        self.setup_fit(confset, target_amplitudes, ref_site, noise_tilde[ref_site], alpha[ref_site, :].copy())
        if opt_noise:
            def ML(x):
                if opt_alpha:
                    if rvm:
                        np.copyto(self.alpha_mat, np.exp(x[1:]))
                    else:
                        self.alpha_mat.fill(np.exp(x[1]))
                self.setup_fit_noise_dep(confset, target_amplitudes, ref_site, np.exp(x[0]), self.alpha_mat)
            ...
            init = [np.log(noise_tilde[ref_site])]

            if opt_alpha:
                bounds.append((np.log(alpha_range[0]), np.log(alpha_range[1])))
                init.append(np.log(alpha[ref_site, 0]))
                if rvm:
                    for i in range(1, alpha.shape[1]):
                        bounds.append((np.log(alpha_range[0]), np.log(alpha_range[1])))
                        init.append(np.log(alpha[ref_site, i]))
            ...
            if opt_alpha:
                if rvm:
                    np.copyto(self.alpha_mat, new_hyperpar[1:])
                else:
                    self.alpha_mat.fill(new_hyperpar[1])

            self.setup_fit_noise_dep(confset, target_amplitudes, ref_site, noise_tilde[ref_site], self.alpha_mat)

        elif opt_alpha:
            for i in range(max_iterations):
                if self.cholesky:
                    gamma = (1 - (self.alpha_mat[self.active_elements])*np.sum(abs(self.Sinv) ** 2, 0))
                else:
                    gamma = (1 - (self.alpha_mat[self.active_elements])*np.diag(self.Sinv).real)
                if rvm:
                    self.alpha_mat[self.active_elements] = (gamma/((self.weights.conj()*self.weights)[self.active_elements])).real
                else:
                    self.alpha_mat.fill((np.sum(gamma)/(self.weights.conj().dot(self.weights))).real)

                self.setup_fit_alpha_dep(confset, target_amplitudes, ref_site, self.alpha_mat)

        alpha[ref_site, :] = self.alpha_mat
