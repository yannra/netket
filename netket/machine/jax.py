...
if utils.flax_available:
    import flax

    def is_flax_module(module):
        return isinstance(module, type) and issubclass(module, flax.nn.Module)

    def wrap_flax_module(module):
        def flax_init(rng, shape):
            out_shape, params = module.init_by_shape(rng, [shape])
            return out_shape.shape, params

        return flax_init, module.call
