...
ma = nk.machine.QGPSLinExp(hi, n_bond_lin=N, n_bond_exp=N, automorphisms=transl, spin_flip_sym=True, cluster_ids=cluster_ids, dtype=complex)
...
if mpi.COMM_WORLD.Get_rank() == 0:
    np.save("epsilon_avg_lin.npy", ma._epsilon_lin)
    np.save("epsilon_avg_exp.npy", ma._epsilon_exp)
